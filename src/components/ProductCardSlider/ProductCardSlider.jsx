...
const ProductCardSlider = () => {
  ...
  const handleNext = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex + 1) % Math.ceil(productList.length / 4)
    );
  };
  ...
  const handlePrev = () => {
    setCurrentIndex(
      (prevIndex) =>
        (prevIndex - 1 + Math.ceil(productList.length / 4)) %
        Math.ceil(productList.length / 4)
    );
  };

  return (
    <div className="ProductCardSlider-container">
      <div className="ProductCardSlider hidden md:block w-5/6 self-center mx-auto">
        <div className="carousel w-full relative">
          ...
          <div className="absolute flex justify-between items-center w-full">
            <button
              className="btn btn-circle border right-3"
              onClick={handlePrev}>
              ❮
            </button>
