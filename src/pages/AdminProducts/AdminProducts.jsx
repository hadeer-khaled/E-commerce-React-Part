...
const AdminProducts = () => {
  ...
  const isLoading = useSelector(
    (state) => state.productsSliceReducer.isLoading
  );
  const categoriesList = useSelector(
    (state) => state.categoriesSliceReducer.categoryList
  );
  const currentPage = useSelector(
    (state) => state.productsSliceReducer.currentPage
  );
  const totalPages = useSelector(
    (state) => state.productsSliceReducer.totalPages
  );
  ...
  const handleDeleteProduct = (product_id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      dispatch(deleteProductByIdThunk({ product_id })).then(() => {
        dispatch(
          getProductsThunk({ page: 1, limit: 10, order: "-product_id" })
        );
      });
    }
  };
  ...
  const handleAddProduct = () => {
    ...
    const imagesString =
      "/cdn.dummyjson.com/product-images/1/1.jpg,/cdn.dummyjson.com/product-images/1/2.jpg,/cdn.dummyjson.com/product-images/1/3.jpg,/cdn.dummyjson.com/product-images/1/4.jpg,/cdn.dummyjson.com/product-images/1/thumbnail.jpg";
  ...
  const handleConfirmUpdate = () => {
    ...
    dispatch(
      updateProductByIdThunk({ product_id: editingProductId, data: newData })
    ).then(() => {
      dispatch(getProductsThunk({ page: 1, limit: 10, order: "-product_id" }));
        ...
        <tbody>
          <tr className="">
            ...
            <td className="border px-2 py-2">
              <button
                onClick={
                  editingProductId ? handleConfirmUpdate : handleAddProduct
                }
                className="w-16 h-8 bg-emerald-500 hover:bg-emerald-700 text-white font-bold rounded mx-auto">
                {editingProductId ? "Confirm" : "Add"}
              </button>
            </td>
          </tr>
            ...
            <tr key={product.product_id}>
              ...
              <td className="border px-2 py-1">
                <button
                  ...
                  className="w-16 h-8 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded m-1">
                  Edit
                </button>
                <button
                  onClick={() => handleDeleteProduct(product.product_id)}
                  className="w-16 h-8 bg-red-500 hover:bg-red-700 text-white font-bold rounded m-1">
                  Delete
                </button>
              </td>
            </tr>
